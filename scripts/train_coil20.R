...
source("scripts/model_coil20.R") # This builds the model

#' Initialize session
session <- tf$Session()
session$run(tf$global_variables_initializer())

load("data/coil20/5pca.RDa")
...
# For plotting:
source("functions/plot_at_iteration_coil20.R")
...
for(i in 1:iterations){
  ...
  if(i %% 1000 == 0){ # Save a model every 1000 iterations
    ...
    plot_at_iteration_coil20(new_z, i)
    filename <- paste("results/coil20/coil20_iteration",i, sep = "")
