...
model$L_scale_matrix <- tf$constant(max(z),dtype=float_type) * initialize_L(model$L_scale_matrix, session = session, B = tf$constant(sample_covariance, dtype = float_type))
