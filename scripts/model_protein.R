...
#' Parameters and more
N <- 6078 # Number of observations
m <- 100 # Number of inducing points
D <- 5 # Ambient dimension / data dimension
d <- 2 # Latent dimension
float_type = tf$float64
#################### Not needed ?
load("data/protein/dists.csv") # CHECK THIS -------------------------------------------------
A <- as.matrix(A)
#####################

load("data/mnist/init_location.RDa") # (initialized with isomap) ---------------------------------------
z <- (z-colMeans(z))/sqrt(max(var(z))) # Scaling here
####################### z could be scaled ?
cut_off <- 7 # CHANGE HERE ----------------------------------------------
#######################
