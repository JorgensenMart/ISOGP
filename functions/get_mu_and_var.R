get_mu_and_var <- function(x_batch,model, K_q = NULL, K_MN = NULL, joint_cov = FALSE){
  if(is.null(K_q)){
    print("hey")
  ...
  if(joint_cov == FALSE){
    ...
    if(S$get_shape()$ndims == 4){ #WISHART (non-constrained)
      ...
      #d_cov <- tf$reduce_sum(tile_A * B, as.integer(2)) #DxNUxN --- summing over M
      K_NN <- build_kernel_matrix(model,x_batch,x_batch,equals = TRUE, only_diag = FALSE) #Nx1
      #out_var <- tf$transpose(tf$transpose(K_NN) + d_cov, as.integer(c(2,0,1))) #NxDxNU
      transpose_tile_A <- tf$transpose(tile_A, as.integer(c(0,1,3,2))) #DxNUxNxM
      out_var <- K_NN + tf$matmul(transpose_tile_A,B) #DxNUxNxN
      out <- list(mean = out_mean, var = out_var)
    } else{
      tile_A <- tf$tile(A[NULL,,], as.integer(c(output_dim,1,1)))
      S <- tf$matmul(S,tf$transpose(S,as.integer(c(0,2,1)))) #DxMxM
      ...
      #d_cov <- tf$reduce_sum(tile_A * B, as.integer(1)) #DxN --- summing over M
      K_NN <- build_kernel_matrix(model,x_batch,x_batch,equals = TRUE, only_diag = FALSE) #Nx1
      transpose_tile_A <- tf$transpose(tile_A, as.integer(c(0,2,1))) #DxNxM
      out_var <- K_NN + tf$matmul(transpose_tile_A,B) # DxNxN
      out <- list(mean = out_mean, var = out_var)
    }
  }
  return(out)
